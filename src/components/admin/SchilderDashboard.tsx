...
import { Paintbrush, Search, Filter, Eye, Mail, Phone, FileText, Calendar, Euro, MapPin, Clock, CheckCircle, AlertCircle } from 'lucide-react';
...
const SchilderDashboard = () => {
  const [requests, setRequests] = useState<SchilderRequest[]>([]);
  const [filteredRequests, setFilteredRequests] = useState<SchilderRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('alle');
  const [selectedRequest, setSelectedRequest] = useState<SchilderRequest | null>(null);
  ...
  const handleStatusUpdate = async (id: string, newStatus: string) => {
    const success = await updateRequestStatus(id, newStatus, 'schilder_aanvragen');
    if (success) {
      loadRequests();
    }
  };

  const handleDetailsUpdate = async (request: SchilderRequest, notes: string, price: string) => {
    const success = await updateRequestDetails(request, notes, price, 'schilder_aanvragen');
    if (success) {
      loadRequests();
    }
  };

  const handleSendQuote = async (request: SchilderRequest, customMessage?: string) => {
    const quoteItem = {
      id: request.id,
      ...
      projectDetails: `${request.project_type} - ${request.oppervlakte}m² - ${request.verf_type}`,
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Weet je zeker dat je deze aanvraag wilt verwijderen?')) {
      const success = await deleteQuote(id, 'schilder_aanvragen');
      if (success) {
        loadRequests();
      }
    }
    ...
    <div className="p-6">
      ...
      <Tabs defaultValue="alle" className="space-y-4">
        ...
        <TabsContent value="alle">
          <Card>
            <CardHeader>
              <CardTitle>Alle Schilderwerk Aanvragen</CardTitle>
            </CardHeader>
            <CardContent>
                      ...
                      <TableRow key={request.id} className="hover:bg-gray-50">
                        ...
                        <TableCell>
                          <div>
                            ...
                            <div className="text-sm text-gray-600">{request.verf_type}</div>
                            {request.gewenste_kleur && (
                              <div className="text-sm text-gray-600">Kleur: {request.gewenste_kleur}</div>
                            )}
                          </div>
                        </TableCell>
