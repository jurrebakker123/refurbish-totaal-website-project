...
const ChatBot = () => {
      ...
      if (!isOpen) {
        toast.info(
          <div className="flex items-center gap-2">
            ...
            <span>{`${getGreeting()} Kan ik u ergens mee helpen?`}</span>
          </div>, 
          {
            action: {
              label: 'Open chat',
  ...
  const handleServiceSelection = (serviceId: string) => {
    ...
    if (selectedService) {
      ...
      setUserResponses({...userResponses, [serviceId]: []});
      
      // Voeg bericht toe dat de dienst is geselecteerd
      setMessages(prev => [...prev, 
        { type: 'user', text: `Ik ben geïnteresseerd in ${selectedService.name}.` },
        { type: 'bot', text: `Bedankt voor uw interesse in ${selectedService.name}. Ik stel u graag een paar vragen om u beter te kunnen helpen.` }
      ]);
      ...
      if (selectedService.questions && selectedService.questions.length > 0) {
        setTimeout(() => {
          ...
          setMessages(prev => [...prev, { type: 'bot', text: firstQuestion.question }]);
        ...
        setOptions([
          {
            ...
            action: () => {
              setMessages(prev => [...prev, 
                { type: 'user', text: 'Ik wil graag een offerte aanvragen.' },
                { type: 'bot', text: 'Perfect! Ik verwijs u door naar ons offerteformulier, waar uw gegevens alvast zijn ingevuld op basis van onze conversatie.' }
              ]);
              setTimeout(() => {
                setIsOpen(false);
          ...
          },
          {
            ...
            action: () => {
              setMessages(prev => [...prev, 
                { type: 'user', text: 'Ik heb liever telefonisch contact.' },
                { type: 'bot', text: 'Natuurlijk! U kunt ons bereiken op +31 6 30136079. We staan u graag te woord op werkdagen tussen 08:00 en 17:00 uur.' }
              ]);
          ...
          },
          {
            ...
            action: () => {
              setMessages(prev => [...prev, 
                { type: 'user', text: 'Ik wil er nog over nadenken.' },
                { type: 'bot', text: 'Geen probleem! Neem gerust de tijd om erover na te denken. Heeft u nog andere vragen waar ik u mee kan helpen?' }
              ]);
    ...
    setTimeout(() => {
      ...
      if (optionText.includes('offerte')) {
        ...
        newOptions = diensten.map(dienst => ({
          ...
          action: () => {
            setMessages(prev => [...prev, 
              { type: 'user', text: `Ik wil meer weten over ${dienst.name}.` },
              { type: 'bot', text: `Ik verwijs u door naar onze pagina over ${dienst.name.toLowerCase()} waar u alle details kunt vinden.` }
            ]);
            setTimeout(() => {
              setIsOpen(false);
        ...
        newOptions = [
          {
            ...
            action: () => {
              setMessages(prev => [...prev, 
                ...
                { type: 'bot', text: 'U kunt ons bereiken op +31 6 30136079. Wij zijn beschikbaar op werkdagen tussen 08:00 en 17:00 uur en helpen u graag verder.' }
              ]);
              setOptions([]);
              setShowOptions(false);
            }
          },
          {
            ...
            action: () => {
              setMessages(prev => [...prev, 
                ...
                { type: 'bot', text: 'U kunt ons een bericht sturen via WhatsApp op nummer +31 6 30136079. We reageren doorgaans binnen enkele uren.' }
              ]);
              setOptions([]);
              setShowOptions(false);
            }
        ...
        newOptions = [
          {
            ...
            action: () => {
              setMessages(prev => [...prev, 
                { type: 'user', text: 'Ik wil graag een afspraak op locatie.' },
                { type: 'bot', text: 'Kunt u aangeven wat voor project het betreft, zodat we de juiste expert kunnen sturen?' }
              ]);
          ...
          },
          {
            ...
            action: () => {
              setMessages(prev => [...prev, 
                { type: 'user', text: 'Ik wil graag telefonisch contact.' },
                { type: 'bot', text: 'Vult u alstublieft hieronder uw telefoonnummer in, dan bellen wij u op een moment dat het u schikt.' }
              ]);
              setTimeout(() => {
                setMessages(prev => [...prev, 
          ...
          setOptions([
            {
              ...
              action: () => {
                setMessages(prev => [...prev, 
                  { type: 'user', text: `Ik wil meer informatie over ${dienst.name}.` },
                  { type: 'bot', text: `Ik verwijs u door naar onze pagina over ${dienst.name.toLowerCase()}.` }
                ]);
                setTimeout(() => {
                  setIsOpen(false);
                  window.location.href = `/diensten/${dienst.id}`;
  ...
  return (
    <>
      ...
      <motion.button 
        onClick={() => setIsOpen(true)}
        className={`fixed bottom-6 right-6 w-16 h-16 rounded-full bg-brand-darkGreen text-white flex items-center justify-center shadow-lg z-30 ${isOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            className="fixed bottom-6 right-6 w-[350px] sm:w-[380px] h-[500px] bg-white rounded-lg shadow-xl flex flex-col z-40"
          ...
          >
            ...
            <div className="bg-brand-darkGreen text-white p-4 rounded-t-lg flex justify-between items-center">
              ...
              <motion.button 
                ...
                className="hover:bg-brand-darkGreen/50 p-1 rounded-full transition-colors"
                  ...
                  >
                    {options.map((option, index) => (
                      <motion.button
                        ...
                        whileHover={{ backgroundColor: "#f8f8f8", x: 2 }}
                        whileTap={{ scale: 0.98 }}
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
            ...
            <div className="p-2 border-t border-gray-200 bg-gray-50 flex justify-center space-x-4">
              <motion.a 
                ...
                whileHover={{ scale: 1.1, backgroundColor: "#1B5E20", color: "#ffffff" }}
                whileTap={{ scale: 0.9 }}
              >
                <Phone size={18} />
              </motion.a>
            ...
            <form onSubmit={handleSubmit} className="p-3 border-t border-gray-200 bg-white rounded-b-lg flex items-center">
              ...
              <motion.button
                ...
                whileHover={{ backgroundColor: "rgba(27, 94, 32, 0.9)" }}
                whileTap={{ scale: 0.95 }}
              >
                <Send size={20} />
              </motion.button>
