...
const ChatBot = () => {
  ...
  return <>
      {/* Chat Icon */}
      <motion.button onClick={() => setIsOpen(true)} className={`fixed bottom-6 right-6 w-16 h-16 rounded-full bg-brand-darkGreen text-white flex items-center justify-center shadow-lg z-30 ${isOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`} initial={{
      scale: 0.8,
      opacity: 0
    }} animate={{
      scale: 1,
      ...
      {/* Chat Window */}
      <AnimatePresence>
        {isOpen && <motion.div className="fixed bottom-6 right-6 w-[350px] sm:w-[380px] h-[500px] bg-white rounded-lg shadow-xl flex flex-col z-40" variants={chatAnimation} initial="hidden" animate="visible" exit="exit">
            {/* Header */}
